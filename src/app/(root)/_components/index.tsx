...
import { Link as Linkit } from "lucide-react";
...
import { SidebarProvider } from "./collapsprovider";
import dynamic from "next/dynamic";

const Doctree =  dynamic(() => import('./doctree'), { ssr: false });
  ...
  const handleCreateDocument = async ({
    ...
    try {
      ...
      const data = parentDocumentId
        ? { title: "Untitled", parentDocumentId }
        : {
            title: "Untitled",
          };
  ...
  const handleMovetoTrash = async (doc: DocumentType) => {
    try {
      ...
      if (doc.parentDocumentId) {
        ...
        if (
          parentGroup &&
          parentGroup.data.filter(
            (d) => d.parentDocumentId === doc.parentDocumentId
          ).length === 0
        ) {
          dispatch(
            updateChildFlag({ parentId: doc.parentDocumentId, child: false })
          );
        }
      }
      if (doc.parentDocumentId) {
          ...
          docs.map((doc, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={cn(
                    "group/item cursor-pointer bg-gray-200 dark:bg-[#FFFFFF0E] p-1 rounded-md justify-between flex",
                    level > 0 && `mt-2`
                  )}
                ...
                >
                  <span className="flex gap-2 items-center">
                    <Avatar
                      className="rounded-md bg-[#191919] items-center justify-center"
                      onClick={() => toggleExpand(doc.id)}
                    >
                      <AvatarImage
                        src="/icons/arrow-left.svg"
                        className={cn(
                          "invert opacity-0 transition duration-300 rotate-180 h-5 w-5 group-hover/item:opacity-100",
                          isExpanded && "rotate-[275deg]"
                        )}
                        alt="Kelly King"
                      />
                      {doc.icon ? (
                        doc.icon.startsWith("data:image") ? (
                          <Image
                            ...
                            src={doc.icon}
                            alt="icon"
                            className="w-14 h-14 object-contain absolute transition duration-300 group-hover/item:opacity-0 font-emoji"
                          />
                        ) : (
                          <p className="absolute transition duration-300 group-hover/item:opacity-0 text-xl font-emoji">
                            {doc.icon}
                          </p>
                        )
                      ) : (
                        <File
                          size={24}
                          className={cn(
                            "absolute transition duration-300 group-hover/item:opacity-0"
                          )}
                        />
                      )}
                    </Avatar>
                    <p
                      className="w-full h-full flex items-center overflow-hidden whitespace-nowrap text-ellipsis truncate"
                      onClick={() => route.push(`/${doc.id}`)}
                      title={doc.title}
                    >
                      {doc.title}
                    </p>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            variant={"ghost"}
                            size={"icon"}
                            className="size-6 cursor-pointer dark:hover:bg-neutral-900"
                            onClick={() =>
                              handleCreateDocument({ parentDocumentId: doc.id })
                            }
                          >
                            <Plus />
                          </Button>
      ...
      >
        <div
          className={cn(
            ...
            "gap-2 p-2 flex flex-row items-center justify-between h-11 m-1 px-2"
          )}
        >
          <UserModal
            detailed={true}
            user={user}
            signOut={signOut}
            className="hover:!bg-none !bg-transparent !px-0"
            variant={"default"}
          />
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant={"ghost"}
                    size={"icon"}
                    className="hidden group-hover:flex hover:dark:!bg-neutral-900"
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className="hover:dark:!bg-neutral-900"
                  >
      ...
      >
        <header className="flex h-12 shrink-0 items-center gap-2 px-2 group/menuhover">
          ...
          <Separator
            ...
            className={cn(
              "mr-2 data-[orientation=vertical]:h-4 hidden",
              isCollapsed && "flex"
            )}
          />
        </header>
