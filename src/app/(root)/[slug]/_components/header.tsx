...
import { useSidebar } from "../../_components/collapsprovider";
...
const Header = ({
  ...
  return (
    <div
      className={cn(
        "absolute -top-10 left-0 w-full h-8 flex px-2 items-center justify-between ",
        isCollapsed && "left-14 pr-14"
      )}
    >
        ...
        >
          <IconPicker onChange={(emoji) => setValue({ icon: emoji })} asChild>
            <Button
              variant="outline"
              size="icon"
              className="text-muted-foreground text-xl font-emoji overflow-hidden"
            >
              {renderIcon()}
            </Button>
          </IconPicker>
      ...
      <span className="flex gap-2">
        ...
        <Popover>
          ...
          <PopoverContent className="w-96 p-2 rounded-xl">
            <Tabs defaultValue="share" className="items-start">
              <TabsList>
                <TabsTrigger value="share">Share</TabsTrigger>
                <TabsTrigger value="public">Publish</TabsTrigger>
              </TabsList>
              <TabsContent
                value="share"
                className="items-center w-full flex justify-center flex-col gap-2"
              >
